...
HMaterialFlatButtonStateMachine::HMaterialFlatButtonStateMachine(HMaterialFlatButton *parent)
      ...
      m_wasChecked(false)
{
    ...
    transition = new HMaterialStateTransition(FlatButtonCheckedTransition);
    ...
    transition = new HMaterialStateTransition(FlatButtonUncheckedTransition);
    ...
    transition = new HMaterialStateTransition(FlatButtonPressedTransition);
...
HMaterialFlatButtonStateMachine::~HMaterialFlatButtonStateMachine()
{
}

void HMaterialFlatButtonStateMachine::setOverlayOpacity(qreal opacity)
{
    m_overlayOpacity = opacity;
    m_button->update();
}

void HMaterialFlatButtonStateMachine::setCheckedOverlayProgress(qreal progress)
{
    m_checkedOverlayProgress = progress;
    m_button->update();
}

void HMaterialFlatButtonStateMachine::setHaloOpacity(qreal opacity)
{
    m_haloOpacity = opacity;
    m_button->update();
}

void HMaterialFlatButtonStateMachine::setHaloSize(qreal size)
{
    m_haloSize = size;
    m_button->update();
}

void HMaterialFlatButtonStateMachine::setHaloScaleFactor(qreal factor)
{
    m_haloScaleFactor = factor;
    m_button->update();
}

void HMaterialFlatButtonStateMachine::startAnimations()
{
    m_haloAnimation->start();
    start();
}

void HMaterialFlatButtonStateMachine::setupProperties()
{
    QColor overlayColor;
...
void HMaterialFlatButtonStateMachine::updateCheckedStatus()
{
    const bool checked = m_button->isChecked();
    if (m_wasChecked != checked) {
        m_wasChecked = checked;
        if (checked) {
            postEvent(new HMaterialStateTransitionEvent(FlatButtonCheckedTransition));
        } else {
            postEvent(new HMaterialStateTransitionEvent(FlatButtonUncheckedTransition));
        }
    }
}

bool HMaterialFlatButtonStateMachine::eventFilter(QObject *watched, QEvent *event)
{
    if (QEvent::FocusIn == event->type()) {
        QFocusEvent *focusEvent = static_cast<QFocusEvent *>(event);
        if (focusEvent && Qt::MouseFocusReason == focusEvent->reason()) {
            postEvent(new HMaterialStateTransitionEvent(FlatButtonPressedTransition));
            return true;
        }
    }
...
void HMaterialFlatButtonStateMachine::addTransition(QObject *object,
...
void HMaterialFlatButtonStateMachine::addTransition(QAbstractTransition *transition,
                                                    QState *fromState,
                                                    QState *toState)
{
